...
import { Building2, Users, BarChart2, Plus, Eye, Clock, CheckCircle2, AlertCircle } from "lucide-react";
...
export default function PortalDashboardHome() {
  ...
  const activities: Array<{ id: string; icon: React.ElementType; text: string; time: string; color: string }> = [];
  ...
  properties.slice(0, 2).forEach((p) => {
    activities.push({
      id: `prop-${p.id}`,
      icon: p.portal_statut === "actif" ? CheckCircle2 : p.portal_statut === "en_attente_validation" ? Clock : Building2,
      text: p.portal_statut === "actif"
    ...
    <PortalLayout title="Tableau de bord">
      <div className="space-y-8">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}
            ...
            <div>
              <p className="text-sm font-semibold text-amber-800">
                {pendingProps.length} bien{pendingProps.length > 1 ? "s" : ""} en attente de validation WeHome
              </p>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <KpiCard icon={Building2} label="Biens actifs" value={`${activeProps.length} / ${limit}`}
            sub="Votre quota du plan" color="text-primary" delay={0.05} />
          <KpiCard icon={Users} label="Leads ce mois" value={stats?.leadsThisMonth ?? 0}
            sub="Contacts acheteurs" color="text-blue-500" delay={0.1} />
          <KpiCard icon={Eye} label="Vues totales" value={stats?.totalViews ?? 0}
            sub="Toutes les annonces" color="text-purple-500" delay={0.15} />
          <KpiCard icon={BarChart2} label="Taux de contact"
            value={stats ? `${stats.conversionRate}%` : "—"}
            sub="Leads / Vues" color="text-green-500" delay={0.2} />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
            ...
            <h2 className="text-sm font-display font-bold text-foreground uppercase tracking-wide">Actions rapides</h2>
            ...
            <Link href="/espace-agent/dashboard/leads">
              <div className="flex items-center gap-4 p-4 bg-white border border-border rounded-2xl hover:border-primary/30 transition-colors cursor-pointer">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground text-xs">{leads.length} contact{leads.length !== 1 ? "s" : ""} total</p>
                </div>
              </div>
            </Link>
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
            ...
            {activities.length === 0 ? (
              <div className="text-center py-10">
                ...
                <p className="text-sm text-muted-foreground">Publiez votre premier bien pour voir l'activité.</p>
                <Link href="/espace-agent/dashboard/biens"
              ...
              <div className="space-y-1">
                {activities.map((a) => (
                  <div key={a.id} className="flex items-start gap-3 px-3 py-2.5 rounded-xl hover:bg-muted/50 transition-colors">
        ...
        {properties.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-display font-bold text-foreground uppercase tracking-wide">Derniers biens</h2>
              <Link href="/espace-agent/dashboard/biens" className="text-xs text-primary font-semibold hover:underline">
                Voir tout →
              </Link>
            </div>
                    ...
                    <div className="flex items-center gap-4 bg-white border border-border rounded-xl p-3.5 hover:border-primary/30 transition-colors cursor-pointer">
                      <div className="w-12 h-12 rounded-xl bg-muted shrink-0 overflow-hidden">
                        {p.photo_principale
                          ...
                          : <div className="w-full h-full flex items-center justify-center"><Building2 size={18} className="text-muted-foreground" /></div>}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-foreground truncate">{p.titre ?? "Sans titre"}</p>
                        <p className="text-xs text-muted-foreground truncate">{p.adresse ?? p.ville ?? "—"}</p>
                      </div>
                      <span className={`text-xs font-semibold px-2.5 py-1 rounded-full shrink-0 ${statusColors[st] ?? "bg-muted text-muted-foreground"}`}>
                        {statusLabels[st] ?? st}
                      </span>
                    </div>
                  </Link>
