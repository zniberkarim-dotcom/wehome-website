...
import {
  fetchPortalProperties,
  createPortalProperty,
  archivePortalProperty,
  uploadPortalPropertyPhoto,
  type PortalProperty,
...
function StatusBadge({ statut }: { statut: string }) {
  return (
    <span
      className={`inline-flex items-center text-xs font-semibold px-2.5 py-1 rounded-full border ${STATUS_COLORS[statut] ?? "bg-muted text-muted-foreground border-border"}`}
    >
...
function NewBienModal({
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.adresse.trim()) {
      setError("Le quartier/adresse est requis.");
      return;
    }
    if (!form.prix) {
    ...
    if (!form.mandat_signe) {
      setError("Vous devez confirmer avoir un mandat signé.");
      return;
    }
    setError(null);
    mutation.mutate();
        ...
        <form onSubmit={handleSubmit} className="px-7 py-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Surface (m²)">
              <Input
                type="number"
                value={form.surface_construite}
                onChange={(e) => set("surface_construite", e.target.value)}
                placeholder="85"
              />
            </Field>
          </div>
          ...
          <Field label="Description">
            <textarea
              ...
              rows={3}
              placeholder="Décrivez le bien, ses points forts, l'environnement…"
          ...
          <Field label="Photos">
            ...
            <p className="text-xs text-muted-foreground">
              Photos professionnelles requises pour validation.
            </p>
          </Field>
          ...
          <label className="flex items-start gap-3 cursor-pointer p-4 rounded-xl border-2 border-dashed border-amber-200 bg-amber-50/50">
            <div className="mt-0.5">
              <input
                ...
                onChange={(e) => set("mandat_signe", e.target.checked)}
                className="sr-only"
              />
              <div
            ...
            <p className="text-sm text-amber-900">
              <strong>Mandat signé :</strong> Je confirme avoir un mandat de vente/location signé
              avec le propriétaire pour ce bien.
            </p>
          </label>
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              {activeCount} actif{activeCount !== 1 ? "s" : ""} · {limit - activeCount} slot
              {limit - activeCount !== 1 ? "s" : ""} disponible
              {limit - activeCount !== 1 ? "s" : ""}
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              disabled={activeCount >= limit}
              title={
                activeCount >= limit ? "Limite de votre plan atteinte" : "Saisie manuelle classique"
              }
        ...
        <div className="bg-white border border-border rounded-2xl p-4">
          <div className="flex items-center justify-between mb-2 text-sm">
            <span className="font-medium text-foreground">
              Quota du plan {agent?.abonnement ?? "essai"}
            </span>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {p.portal_statut === "actif" && (
                      <Link href={`/bien/${p.id}`} target="_blank">
                        <button
                          className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-muted transition-colors text-muted-foreground hover:text-primary"
                    ...
                    {(p.portal_statut === "actif" ||
                      ...
                      <button
                        onClick={() => {
                          if (confirm("Archiver ce bien ?")) archiveMutation.mutate({ id: p.id! });
                        }}
