    ...
    <DashboardLayout title="Tableau de bord">
      <div className="space-y-8">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="text-muted-foreground mt-1">Gérez vos biens et votre profil depuis cet espace.</p>
        </motion.div>

        <div className="grid grid-cols-3 gap-4">
          {stats.map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
              transition={{ delay: i * 0.08 }} className="bg-card border border-border rounded-2xl p-5">
        ...
        {properties.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/dashboard/proprietes" className="text-sm text-primary font-semibold hover:underline">Voir tout</Link>
            </div>
                  ...
                  <div className="flex items-center gap-4 bg-card border border-border rounded-2xl p-4 hover:border-primary/40 transition-colors cursor-pointer">
                    <div className="w-12 h-12 rounded-xl overflow-hidden bg-secondary shrink-0">
                      {p.photo_principale
                        ...
                        : <div className="w-full h-full flex items-center justify-center"><Building2 size={20} className="text-muted-foreground" /></div>}
                    </div>
                    ...
                    <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${p.actif !== false ? "bg-green-100 text-green-700" : "bg-muted text-muted-foreground"}`}>
