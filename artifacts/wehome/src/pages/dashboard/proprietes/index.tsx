...
import { Plus, Building2, Pencil, Trash2, ToggleLeft, ToggleRight, Loader2, Eye } from "lucide-react";
      ...
      <div className="space-y-6">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">{properties.length} bien{properties.length !== 1 ? "s" : ""} au total</p>
          </div>
        ...
        {isLoading ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            ...
            <p className="text-muted-foreground text-sm mb-6">Commencez par ajouter votre premier bien.</p>
            <Link href="/dashboard/proprietes/new">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border/60">
                    <th className="text-left px-5 py-4 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Bien</th>
                    <th className="text-left px-4 py-4 text-xs font-semibold text-muted-foreground uppercase tracking-wider hidden md:table-cell">Prix</th>
                    <th className="text-left px-4 py-4 text-xs font-semibold text-muted-foreground uppercase tracking-wider hidden sm:table-cell">Type</th>
                    <th className="text-left px-4 py-4 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Statut</th>
                  ...
                  {properties.map((p, i) => (
                    <motion.tr key={p.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <p className="text-sm font-semibold text-foreground truncate max-w-[160px]">{p.titre ?? "Sans titre"}</p>
                            <p className="text-xs text-muted-foreground truncate max-w-[160px]">{p.adresse ?? "—"}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-4 hidden sm:table-cell">
                        <span className="text-xs px-2.5 py-1 rounded-full bg-secondary text-foreground/70 font-medium">{p.type ?? "—"}</span>
                      </td>
                      <td className="px-4 py-4">
                        <button onClick={() => handleToggle(p)} disabled={togglingId === p.id} className="flex items-center gap-1.5">
                          {togglingId === p.id
                            ? <Loader2 size={16} className="animate-spin text-muted-foreground" />
                            : p.actif !== false
                              ? <ToggleRight size={20} className="text-green-600" />
                              : <ToggleLeft size={20} className="text-muted-foreground" />}
                          <span className={`text-xs font-semibold ${p.actif !== false ? "text-green-700" : "text-muted-foreground"}`}>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center justify-end gap-2">
                          <a href={`/bien/${p.id}`} target="_blank"
