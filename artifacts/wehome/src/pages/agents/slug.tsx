          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
              <div className="w-24 h-24 rounded-full overflow-hidden bg-secondary shrink-0">
                {agent.photo_url ? (
                  <img src={agent.photo_url} alt={`${agent.prenom} ${agent.nom}`} className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full flex items-center justify-center text-white text-3xl font-display font-bold" style={{ background: "#8B1A2E" }}>
                ...
                {agent.specialites && agent.specialites.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-3">
                    {agent.specialites.map((s) => (
                      <span key={s} className="text-xs font-semibold px-3 py-1 rounded-full bg-secondary text-foreground/70">{s}</span>
                    ))}
                  </div>
                )}
                {agent.bio && <p className="text-muted-foreground mt-4 leading-relaxed max-w-2xl">{agent.bio}</p>}
            ...
            {propsLoading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {properties.map((property, i) => (
                  <motion.div key={property.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 + i * 0.06 }}>
